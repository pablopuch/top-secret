...
URL_BASE = "https://www.coches.net/search/?hasPhoto=false&wwa=false"
...
def scrape_coches_net(max_paginas=5, pagina_inicio=1, resultados_previos=None):
    ...
    with sync_playwright() as p:
        ...
        try:
            for pagina in range(pagina_inicio, max_paginas + 1):
                if pagina > 1:
                    url_pagina = f"https://www.coches.net/search/?Section1Id=2500&pg={pagina}&hasPhoto=false&wwa=false"
                    try:
                ...
                # Crítico para ejecuciones de miles de páginas.
                if pagina % 5 == 0:
                    with open("coches_net_seguridad.json", "w", encoding="utf-8") as f:
...
if __name__ == "__main__":
    print("Iniciando scraper de coches.net...")
    
    # 1. Cargamos lo que ya tenías guardado de antes
    archivo_json = "coches_net.json"
    inventario_previo = []
    ...
    # 2. Cambiamos la página de inicio a la 98 para reanudar justo donde lo dejaste
    inventario = scrape_coches_net(max_paginas=8614, pagina_inicio=264, resultados_previos=inventario_previo) 
    
    with open(archivo_json, "w", encoding="utf-8") as f:
        json.dump(inventario, f, indent=4, ensure_ascii=False)
        
    print(f"\n✅ Total final: {len(inventario)} vehículos guardados en {archivo_json}")
